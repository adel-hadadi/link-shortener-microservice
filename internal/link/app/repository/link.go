...
func (r LinkRepository) GetByOriginalLink(ctx context.Context, originalLink string) (service.Link, error) {
	ctx, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()

	query := `SELECT * FROM links WHERE original_link=$1`

	// TODO: change this structure to database model
	link := service.Link{}
